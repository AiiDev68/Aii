...
class _AdminPageState extends State<AdminPage> {
  ...
  Future<void> _fetchUsers() async {
    ...
    try {
      final res = await http.get(Uri.parse('Api.api/api/user/listUsers?key=$sessionKey'));
  ...
  Future<void> _deleteUser(String username) async {
    setState(() => isLoading = true);
    try {
      final res = await http.get(Uri.parse('Api.api/api/user/deleteUser?key=$sessionKey&username=$username'));
      final data = jsonDecode(res.body);
  ...
  Future<void> _createAccount() async {
    ...
    try {
      final url = Uri.parse('Api.api/api/user/userAdd?key=$sessionKey&username=$username&password=$password&day=$day&role=$newUserRole');
  ...
  Widget _buildCard({required String title, required IconData icon, required VoidCallback onTap}) {
    return InkWell(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            Icon(icon, color: const Color(0xFF00F0FF), size: 32),
            const SizedBox(height: 12),
            Text(title, style: const TextStyle(color: Color(0xFF00F0FF), fontSize: 16, fontWeight: FontWeight.w500)),
          ],
        ),
      ),
    );
    ...
    return Wrap(
      ...
      children: roleOptions.map((role) {
        ...
        return FilterChip(
          ...
          labelStyle: TextStyle(color: isSelected ? Colors.black : const Color(0xFF00F0FF)),
          selected: isSelected,
          ...
          side: BorderSide(color: isSelected ? const Color(0xFF00F0FF) : Colors.grey.shade600),
        );
      }).toList(),
    );
  }
          ...
          return Container(
            ...
            child: Row(
              children: [
                ...
                // Action
                IconButton(
                  icon: const Icon(Icons.delete_outline, color: Color(0xFF00F0FF), size: 18),
  ...
  Color _getRoleColor(String role) {
    switch (role.toLowerCase()) {
      ...
      default:
        return const Color(0xFF00F0FF); // Changed from Colors.grey to our green color
    }
  }
  ...
  Widget _buildPaginationControls() {
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          IconButton(
            icon: const Icon(Icons.chevron_left, color: Color(0xFF00F0FF)),
          ...
          IconButton(
            icon: const Icon(Icons.chevron_right, color: Color(0xFF00F0FF)),
    ...
    return Dialog(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            const Text('Create New User', style: TextStyle(color: Color(0xFF00F0FF), fontSize: 20, fontWeight: FontWeight.bold)),
      ...
      builder: (_) => Dialog(
        ...
        child: Padding(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              const Text('Delete User', style: TextStyle(color: Color(0xFF00F0FF), fontSize: 20, fontWeight: FontWeight.bold)),
              const SizedBox(height: 20),
  ...
  void _showDeleteConfirmationDialog(String username) {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        backgroundColor: Colors.grey[900],
        title: const Text('Confirm Delete', style: TextStyle(color: Color(0xFF00F0FF))),
        ...
        actions: [
          ...
          TextButton(
            ...
            child: const Text('Delete', style: TextStyle(color: Color(0xFF00F0FF))),
          ),
        ],
      ),
    );
  ...
  InputDecoration _inputDecoration(String label) {
    return InputDecoration(
      labelText: label,
      labelStyle: TextStyle(color: const Color(0xFF00F0FF).withOpacity(0.7)),

...
class _SenderPageState extends State<SenderPage> with TickerProviderStateMixin {
  ...
  static const Color primaryColor = Color(0xFF00F0FF); // Changed from purple to green
  static const Color accentColor = Color(0xFF6EE7B7); // Lighter green for accent
  static const Color secondaryColor = Color(0xFF0E1530);
              ...
              Row(
                children: [
                  Container(
                    ...
                    child: const Icon(Icons.phonelink_lock, color: Color(0xFF00F0FF)),
                  ),
                  const SizedBox(width: 12),
                  const Text(
                    "Pairing Code",
          ...
          return Container(
            ...
            child: ListTile(
              ...
              leading: Container(
                ...
                child: Icon(
                  Icons.phone,
                  color: isGlobal ? accentColor : const Color(0xFF00F0FF),
                ),
              ),
              title: Text(
...
class ApiService {
  static Future<Map<String, dynamic>> getMySender(String sessionKey) async {
    try {
      final response = await http.get(
        Uri.parse("${Api.api}/api/whatsapp/mySender?key=$sessionKey"),
      );
  ...
  static Future<Map<String, dynamic>> getPairing(String sessionKey, String number) async {
    try {
      final response = await http.get(
        Uri.parse("${Api.api}/api/whatsapp/getPairing?key=$sessionKey&number=$number"),
      );

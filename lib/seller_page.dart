...
class _SellerPageState extends State<SellerPage> {
  ...
  Future<void> _create() async {
    ...
    try {
      final res = await http.get(Uri.parse(
          "${Api.api}/api/user/createAccount?key=${widget.keyToken}&newUser=$u&pass=$p&day=$d"));
  ...
  Future<void> _edit() async {
    ...
    try {
      final res = await http.get(Uri.parse(
          "${Api.api}/api/user/editUser?key=${widget.keyToken}&username=$u&addDays=$d"));
      final data = jsonDecode(res.body);
      if (data['edited'] == true) {
    ...
    return InkWell(
      ...
      child: Container(
        ...
        child: Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(icon, color: const Color(0xFF00F0FF), size: 28),
            const SizedBox(width: 16),
            Text(title, style: const TextStyle(color: Color(0xFF00F0FF), fontSize: 18, fontWeight: FontWeight.w600)),
          ],
        ),
      ),
    );
    ...
    return Dialog(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            Text(title, style: const TextStyle(color: Color(0xFF00F0FF), fontSize: 20, fontWeight: FontWeight.bold)),
  ...
  Widget _inputField(String label, TextEditingController c, {TextInputType type = TextInputType.text}) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(label, style: const TextStyle(color: Color(0xFF00F0FF), fontSize: 14)),
